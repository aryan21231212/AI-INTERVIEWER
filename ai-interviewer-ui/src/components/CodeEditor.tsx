...
// Notice the updated interface matching our new rich backend data!
...
export default function CodeEditor({ problem, onCodeRun }: CodeEditorProps) {
  ...
  const handleRunCode = async () => {
    ...
    try {
      const response = await fetch('https://eval-ai-3.onrender.com/api/execute', {
        method: 'POST',
  ...
  return (
    <div className="flex h-full w-full bg-gray-900 border-l border-gray-800">
      ...
      {/* Left Panel: Dynamic LeetCode Problem Statement */}
      ...
      <div className="w-3/5 flex flex-col">
        <div className="h-14 flex items-center justify-between px-4 bg-gray-800/40 border-b border-gray-800">
          <div className="bg-gray-900 border border-gray-700 text-sm text-gray-300 rounded-md px-3 py-1.5">
            JavaScript
          </div>
          <div className="flex gap-3">
            <button onClick={handleRunCode} disabled={isRunning} className="flex items-center gap-2 bg-gray-700 hover:bg-gray-600 text-gray-200 px-4 py-1.5 text-sm rounded-md transition-colors">
              {isRunning ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} className="text-green-400" />} Run Code
            </button>
            <button className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white px-4 py-1.5 text-sm rounded-md transition-colors">
            ...
            {results && (
              <div className="flex gap-2 mb-4">
                {results.map((res, idx) => (
                  ...
                  >
                    {res.passed ? <CheckCircle2 size={12} className="text-green-400" /> : <XCircle size={12} className="text-red-400" />}
                    Case {idx + 1}
                  </button>
                ))}
              </div>
            ...
            <div className="bg-black/30 rounded-lg p-4 font-mono text-sm border border-gray-800/50 flex-1 overflow-y-auto">
              {!results ? (
                <div className="text-gray-400 flex h-full items-center justify-center italic">
                  Run your code to execute test cases against the backend.
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
                    <div className="text-gray-500 text-xs mb-1 font-sans">Input Context</div>
                  ...
                  <div>
                    ...
                    <div className={results[activeTab]?.passed ? "text-green-400" : "text-red-400"}>{results[activeTab]?.actual}</div>
                  </div>
                  {results[activeTab]?.error && (
                    <div className="text-red-400 text-xs mt-2 bg-red-900/20 p-2 rounded">{results[activeTab]?.error}</div>
                  )}
                </div>
              )}
            </div>
